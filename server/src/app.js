...
const wss = new Server(httpServer, { cors: {
...
wss.on('connection', playerSocket => {
	playerSocket.on('init', (userData) => {
		onlineUsers.set(playerSocket.id, userData)
		console.log('inited', userData.nickname, '!')
	})
	playerSocket.on("join_to_room", (roomID) => {
		playerSocket.join(roomID)
		console.log(`${onlineUsers.get(playerSocket.id).nickname} joined to room ${roomID}!!!`)
		console.log('All rooms', wss.sockets.adapter.rooms)
	})
	playerSocket.on('disconnecting', (reason) => {
